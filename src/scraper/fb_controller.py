...
class FacebookController(driver.Firefox):
    ...
    def year_selector(self, year: int, wrapper_css_selector: str):
        ...
        for key in year_dict.keys():
            if str(year) == key:
                ...
                btn = options_wrapper.find_elements(
    ...
    def search_posts(
        self, year: Optional[int] = None, search_keyword: Optional[str] = None
    ) -> None:
        if year and search_keyword is None:
            ...
            year_selection = self.find_elements(
            ...
            btn = self.find_element(
                By.CSS_SELECTOR, f'div[aria-label="{self.person_name}님의 게시물 검색"]'
            )
            btn.click()
    ...
    def position_to_base(self, element: WebElement) -> None:
        ...
        current_y = (
            self.execute_script("return window.innerHeight") / 2
    ...
    def saved_file_by_moving(
        ...
        try:
            ...
            if year and not search_keyword:
                ...
                # generator
                year_wrapper_gen = FacebookController.make_wrapper_generator(
                    web_elements=wrappers, dates=links
                )
                if scrape_count >= 1:
                    for _ in range(scrape_count):
                        if year_wrapper_gen:
                            data = next(year_wrapper_gen)
                    ...
                    raise Exception(
                        "Scrape Count should be same or more at least a elem"
                    )
            # print(
            #         f"Wrapper: {scrape_count} are totally saved, MissingBtn:{missing_btn}"
            #     )
                ...
                if scrape_count >= 1:
                    for _ in range(scrape_count):
                        ...
                        if (
                            ...
                            big_wrapper = self.find_element(
                                By.CSS_SELECTOR, 'div[role="article"]'
                            )
                            date = (
                                big_wrapper.find_element(
                                    By.CSS_SELECTOR, 'a[href*="posts"]'
                                )
                            ...
                            data = []
                            posts_wrapper = big_wrapper.find_element(
                                By.CSS_SELECTOR, 'div[data-ad-preview="message"]'
                            )
                            posts = posts_wrapper.find_elements(
